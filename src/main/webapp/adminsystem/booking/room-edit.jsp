			...
			<form class="form" action="../update" method="post">
				<div class="dropdown-input">	
					<div class="form-group">
						...
						<input name="roomtypeName" type="text" id="dropdown" placeholder="請輸入房間類型名稱" value="${room.roomtypeName}" disabled>
					</div>
				</div>

				<div class="form-group">
					<label for="room-number">房間號碼</label> 
					<input type="text" id="room-number" name="roomNumber" placeholder="輸入內容" value="${room.roomNumber}" required>
				</div>

				<div class="form-group">
					<label for="room-description">房間說明</label> <input type="text"
						id="room-description" name="roomDescription" placeholder="輸入內容" value="${room.roomDescription}">
				</div>
				...
				<div class="form-group roomtype-capacity-group room-status-group">
					<div class="choose-room-status">
						<label class="radio-label">閒置<input type="radio"
							name="roomStatus" value="0" ${room.roomStatus == '0' ? 'checked' : ''}>
						</label>
					</div>
					<div class="choose-room-status">
						<label class="radio-label">已預定<input type="radio"
							name="roomStatus" value="1" ${room.roomStatus == '1' ? 'checked' : ''}>
						</label>
					</div>
					<div class="choose-room-status">
						<label class="radio-label">已入住<input type="radio"
							name="roomStatus" value="2" ${room.roomStatus == '2' ? 'checked' : ''}>
						</label>
					</div>
				</div>
